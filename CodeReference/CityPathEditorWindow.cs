...
namespace Ranmaru.GameComponent.City
{
    ...
    /// Cityパスエディタウィンドウ
    /// </summary>
    public class CityPathEditorWindow : CityEditorWindowBase
    {
        private const string WindowName = "City Path Editor";
        ...
        private bool isAddSpecialNode = false;

        /// <summary>
        /// 初期化.
        /// ショットカットキー：⌘ + alt + e
        /// </summary>
        [MenuItem("Window/Ranmaru/City Path Editor %&e")]
        static public void Initialize()
        {
            CityPathEditorWindow window = EditorWindow.GetWindow<CityPathEditorWindow>( true, WindowName );

            Vector2 WindowSize = new Vector2( 400.0f, 300.0f );
        ...
        private void OnGUI()
        {
            ...
            DrawTextBox( "曲線である必要がない場合、負荷軽減の為、直線を使用してください。" );
            ...
            ButtonToggleNodeType();

            GUILayout.Space( SeparateSpace );

            CheckBoxToggleAddSpecialNode();
            ...
            ButtonToggleGenerationNode();

            GUILayout.Space( SeparateSpace );

            ButtonAutomaticAlignmentPoszOfNode();
        ...
        private void ButtonAddNode( PathNodeInfo.PathType pathType )
        {
            ...
            if( GUILayout.Button( buttonName, GUILayout.Height( ButtonDefaultHeight ) ) )
            {
                ...
                // NavigationPathSystemをAttach.
                if( nodeParentObj.GetComponent<NavigationPathSystem>() == null )
                {
                    nodeParentObj.AddComponent<NavigationPathSystem>();
                }
                ...
                nodePos.x += 1.0f;
                nodePos.z = CityController.GetFixedPosZ( nodePos.y );

                // NodeのGameObjectの名前.
                int i = 0;
                for( ; i < allPathNodeObj.Count; ++i )
                {
                    if( allPathNodeObj[ i ].name != TerritoryTools.GetNodeNameByID( ( i + 1 ) ) )
                    {
                        break;
                    }
                }
                string objName = TerritoryTools.GetNodeNameByID( ( i + 1 ) );

                // PathNode生成.
                GameObject obj = TerritoryTools.CreatePathNode(
                    nodeParentObj.transform,
                    nodePos,
                    objName,
                    i + 1,
                    this.isAddSpecialNode == true ? PathNode.NodeType.Special : PathNode.NodeType.Normal,
                    false
                );
        ...
        /// Button : Nodeタイプの切り替え.
        /// </summary>
        private void ButtonToggleNodeType()
        {
            if( GUILayout.Button( "Nodeタイプの切り替え", GUILayout.Height( ButtonDefaultHeight ) ) )
            {
                if( IsPathNodeObjSelected == false )
                {
                    EditorUtility.DisplayDialog( WindowName, "PathNodeを選択してください。", "OK" );
                    return;
                }
                ...
                {
                    PathNode selectedPathNode = selectObj[ i ].GetComponent<PathNode>();
                    if( selectedPathNode != null )
                    {
                        selectedPathNode.Type =
                            selectedPathNode.Type == PathNode.NodeType.Normal ?
                                PathNode.NodeType.Special : PathNode.NodeType.Normal;
                    }

                    MeshRenderer renderer = selectObj[ i ].GetComponent<MeshRenderer>();
                    renderer.material = TerritoryTools.GetNodeMaterial( selectedPathNode.Type );

                    EditorUtility.SetDirty( selectObj[ i ] );
                }
            }
        }

        /// <summary>
        /// CheckBox : 特殊なノードを追加する.
        /// </summary>
        private void CheckBoxToggleAddSpecialNode()
        {
            this.isAddSpecialNode = EditorGUILayout.Toggle( "特殊ノード追加", this.isAddSpecialNode );
        }
                ...
                else
                {
                    ...
                    for( int i = 0; i < nodeInfos.Count; ++i )
                    {
                        if( nodeInfos[ i ].pathType == PathNodeInfo.PathType.Curve )
                        {
                            ...
                            if( nodeInfos[ i ].curveCtrlPointObj != null ) Undo.DestroyObjectImmediate( nodeInfos[ i ].curveCtrlPointObj );
                        }
        ...
        private void ButtonDisconnectNode()
        {
            if( GUILayout.Button( "Node間切断", GUILayout.Height( ButtonDefaultHeight ) ) )
            {
                ...
                else
                {
                    ...
                    if( infoForRemove.curveCtrlPointObj != null ) Undo.DestroyObjectImmediate( infoForRemove.curveCtrlPointObj );
        ...
        {
            DrawTextBox( "Nodeを削除する際にHierarchyで行われず、下記ボタンを使ってください。" );
        ...
        /// <summary>
        /// Button : 生成用ノードかを切り替える
        /// </summary>
        private void ButtonToggleGenerationNode()
        {
            if( GUILayout.Button( "生成用ノードであるかの切り替え", GUILayout.Height( ButtonDefaultHeight ) ) )
            {
                if( !IsPathNodeObjSelected ) return;

                GameObject[] selectObj = Selection.gameObjects;
                for( int i = 0; i < selectObj.Length; ++i )
                {
                    PathNode pathNode = selectObj[ i ].GetComponent<PathNode>();
                    pathNode.GenerationNode = pathNode.GenerationNode ? false : true;
        ...
        /// <summary>
        /// Button : PathNodeのZ位置を自動調整する.
        /// </summary>
        private void ButtonAutomaticAlignmentPoszOfNode()
        {
            DrawTextBox( "Node間のポイント間隔が不一致の場合、下記ボタンを押してください。" );

            if( GUILayout.Button( "NodePosZ自動調整", GUILayout.Height( ButtonDefaultHeight ) ) )
            {
                Object[] pathNodeObj = GameObject.FindObjectsOfType( typeof( PathNode ) );
    
                if( pathNodeObj.Length == 0 )
                {
                    EditorUtility.DisplayDialog( WindowName, "PathNodeが存在しませんので、Nodeを追加してください。", "OK" );
                }
                else
                {
                    for( int i = 0; i < pathNodeObj.Length; ++i )
                    {
                        ...
                        node.transform.position =
                            new Vector3( nodePos.x,
                                         nodePos.y,
                                         CityController.GetFixedPosZ( nodePos.y ) );

                        EditorUtility.SetDirty( node.gameObject );
                    }

                    EditorUtility.DisplayDialog( WindowName, "PathNodeのZ位置を自動調整しました。", "OK" );
                }
            }
        }
        ...
        private void ListButtonAllPathNodeObjcect()
        {
            ...
            DrawTextBox( "下記ボタンを押すとGameObjectが自動的に選択されます。" );
            ...
            while( true )
            {
                ...
                for( columni = 0; columni < columnNum; ++columni )
                {
                    ...
                    if( num >= nodeGameObjects.Count ) break;
                ...
                if( linei * columnNum >= nodeGameObjects.Count ) break;
            }
